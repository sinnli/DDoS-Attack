...
void App::initialize()
{
    ...
    if(isZombie){
        scheduleAt(zombieTime, generatePacket);
    }
    else
...
void App::handleMessage(cMessage *msg)
{
    if (msg == generatePacket) {
        if( myAddress < 100 && myAddress != victim ){ //means this is not a router / victim (server) so generate messages
            // Sending packet
           // int destAddress = destAddresses[intuniform(0, destAddresses.size()-1)];
            int destAddress;

            //if zombie attack only the victim
            if(isZombie){
                destAddress = victim;
            }
            //if not zombie continue regular
            else{
                do{
                    destAddress = intuniform(0,20);
            ...
            send(pk, "out");
            if(isZombie){
                scheduleAt(simTime() + zombieTime, generatePacket);
            }
            else{
