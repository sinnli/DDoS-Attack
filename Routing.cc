...
class Routing : public cSimpleModule
{
  private:
    ...
    cDoubleHistogram throughputStats;
...
void Routing::initialize()
{
    ...
    throughputVector.record(0);
    throughputStats.collect(0);
...
void Routing::handleMessage(cMessage *msg)
{
    ...
    //take statistics
    int numBytes = pk->getByteLength();
    tempBytes += numBytes;
    simtime_t interval = simTime() - prev;

    //take throughput statistics
    if(interval >= 0.1 ){
        double tempThroughput = tempBytes / interval.dbl();
        throughputVector.record(tempThroughput);
        throughputStats.collect(tempThroughput);
        tempBytes = 0;
        prev = simTime();
    }
